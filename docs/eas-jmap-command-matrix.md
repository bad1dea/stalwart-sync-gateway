...
| MeetingResponse | 2.2.1.11 | 2.5 (core fields) / 14.1+ (`InstanceId`) / 16.0-16.1 (`ProposedStartTime`/`ProposedEndTime`/`SendResponse`) | ⛔ | ⛔ — `participants` confirmed NOT supported by Stalwart's `CalendarEvent` at all | Advertised, no handler, 501. **Corrected finding, supersedes the previous session's read of `mayRSVP: true`.** That flag is a generic per-calendar ACL right, not evidence of a working scheduling mechanism — live-tested directly this session: created a throwaway event via `CalendarEvent/set` with a fully self-contained `participants` map (same account as the only participant, no `sendTo`, `expectReply: false` — safe by construction, no third party involved either way). Stalwart's `create` silently accepted the call but dropped `participants` entirely: `CalendarEvent/get` (even with `properties: ["participants", "replyTo"]` explicitly requested) returned neither key at all, not even empty. Confirmed harder: a follow-up `CalendarEvent/set update` patching `participants/p1/participationStatus` failed with `notUpdated: {type: "invalidProperties"}`, since `p1` was never actually stored. This isn't a risk-avoidance deferral anymore — the JMAP backend genuinely doesn't have anywhere to put this data today. Blocked until Stalwart implements `participants` on `CalendarEvent`, not a gateway-side task. Test event created and destroyed cleanly, no side effects. |
