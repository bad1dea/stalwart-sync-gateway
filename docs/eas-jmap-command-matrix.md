...
| Sync | 2.2.1.21 | 2.5 | ✅ *(mail/notes two-way; contacts/calendar read-only)* | `Email`/`ContactCard`/`CalendarEvent` `/query`+`/get`, plus `Email/set` for mail and the synthetic-message workaround for Notes | `sync_mail()` (`activesync.rs:740`), `sync_contacts_collection()` (`activesync.rs:1666`), `sync_calendar_collection()` (`activesync.rs:1871`), `sync_notes_collection()` (`activesync.rs:2054`). **Verified fresh this pass, by direct grep**: neither `sync_contacts_collection` nor `sync_calendar_collection` contains a single `SyncClientCommandKind` match anywhere in their bodies — Add/Change/Delete from the client is silently a no-op for both. Mail's own client-Add (e.g. append-to-Sent) is also explicitly ignored (`apply_mail_client_commands`, logged debug, no-op) — real EAS clients rarely Add mail directly since SendMail covers the compose path, so this is low priority unlike the Contacts/Calendar gap. |
