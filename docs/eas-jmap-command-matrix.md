...
| MeetingResponse | 2.2.1.11 | 2.5 (core fields) / 14.1+ (`InstanceId`) / 16.0-16.1 (`ProposedStartTime`/`ProposedEndTime`/`SendResponse`) | ⛔ *(today)* / 🟡 | ❓ — mechanism now identified, send-side behavior still unverified | Advertised, no handler, 501. High-value gap — calendar sync has zero interaction path currently. Live-checked this session: `Calendar/get`'s `myRights` on the real account includes `"mayRSVP": true`, confirming Stalwart's JMAP has a real scheduling-reply concept. Mechanism is almost certainly `CalendarEvent/set` updating the user's own `participationStatus` within `participants`, which per the JSCalendar/JMAP-Calendars draft should auto-send a real iTIP reply to the organizer — but that send-side effect was deliberately never live-tested (risks emailing a real third party unsupervised, same caution as `Settings>Oof`'s enabled path). Still ❓ pending that specific test; see docs/eas-jmap-gap-analysis.md roadmap item 3 for the exact next step. |
