...
| ItemOperations | 2.2.1.10 | 12.0 | ✅ *(whole-email fetch only)* | `Email/get` | `item_operations()` (`activesync.rs:320`). Only `Store="Mailbox"` is handled, and that branch ONLY fetches a whole email by ServerId (`get_email_by_id` + `write_email_fields`) — confirmed via direct code read there is NO attachment-fetch path here at all (`download_blob()` is never called in this function), correcting an earlier note that assumed it "presumably" handled attachments. Anything else (Notes/Contacts/Calendar item fetch, `DocumentLibrary` store, attachment fetch) returns Status 2 cleanly, not a hang or 400. Real device confirmed this session to use the *Sync-embedded* `<Fetch>` instead for opening messages, and the separate `GetAttachment` command (not this one) for attachment downloads — this command exists for other-client compatibility, not the primary flow for either use case. |
| MeetingResponse | 2.2.1.11 | 2.5 (core fields) / 14.1+ (`InstanceId`) / 16.0-16.1 (`ProposedStartTime`/`ProposedEndTime`/`SendResponse`) | ⛔ | 🟡 — `participants` IS supported (see correction below); the real blocker is protocol-version scope | Advertised, no handler, 501. **The "`participants` confirmed NOT supported" finding two sessions ago was itself wrong and has been corrected (2026-08-25/26) — see `docs/eas-jmap-gap-analysis.md`'s Attendees row.** The original test's `participants` payload was missing a required top-level `replyTo` (JSCalendar RFC 8984 §4.4.1) and per-participant `calendarAddress`; both silently drop the whole map with no error, which is exactly why the earlier test misread "malformed payload" as "unsupported". Fixed and live-verified — Calendar attendees are now two-way (`deploy-2026-08-26l`). What's ACTUALLY still blocking MeetingResponse is a different, real constraint, found by reading the spec text directly: *"When protocol versions 2.5, 12.0, 12.1, or 14.0 are used, MeetingResponse cannot be used to modify meeting requests in the Calendar folder"* (MS-ASCMD 2.2.1.11) — only the Inbox meeting-request email. This gateway advertises `12.0,12.1,14.0` (fixed, standing hard boundary). Building this needs the Inbox-side iTip flow investigated first: how Stalwart surfaces an incoming meeting request over JMAP, and how to correlate a MeetingResponse `Request`'s Inbox `CollectionId`+`RequestId` back to a `CalendarEvent` to patch. Not investigated yet. |
| MoveItems | 2.2.1.12 | 2.5 | ✅ *(mail only)* | `Email/set` (`mailboxIds`) | `move_items()` (`activesync.rs:731`) unconditionally calls `move_email()`, which does a plain `Email/set update` on whatever id it's given — confirmed by direct code read (settled, no longer "unverified"). For a Notes/Contacts/Calendar item, the id passed would be that class's own ServerId shape (e.g. Notes' `noteid-*` stable id, never a real JMAP `Email` id), so `Email/set` would reject it as `notFound` — which `move_items()` already catches and reports back as a clean Status 5, not silent corruption or a hang. Currently moot in practice regardless: every non-mail collection (Notes/Contacts/Calendar) has exactly one folder today, so there's nowhere for a real client to even offer moving an item TO. Worth revisiting only once multiple folders per collection kind exist. |
...
| SmartForward | 2.2.1.19 | 2.5 (WBXML) / 14.0+ (raw-MIME) | ✅ | Same `EmailSubmission/set` path as SendMail | Same handler as SendMail (`command.eq_ignore_ascii_case("SmartForward")` dispatches to `send_mail()`). Threading fidelity live-verified this session: a self-addressed test message's Message-ID/In-Reply-To/References survived byte-identical through the full send→import→deliver path (checked both the Sent Items and delivered Inbox copies via direct JMAP query). Source-confirmed the only MIME mutation anywhere in the pipeline is `rewrite_from_header()`, which touches only the `From:` line. |
| SmartReply | 2.2.1.20 | 2.5 (WBXML) / 14.0+ (raw-MIME) | ✅ | Same as SmartForward | Same story and same live verification as SmartForward — SendMail/SmartForward/SmartReply are one identical code path server-side. |
| Sync | 2.2.1.21 | 2.5 | ✅ *(mail/notes/contacts/calendar/tasks all two-way as of `deploy-2026-08-26i`; calendar recurrence confirmed blocked on Stalwart, not just out of scope)* | `Email`/`ContactCard`/`CalendarEvent` `/query`+`/get`+`/set` (Tasks rides on `CalendarEvent` too, via `@type: "Task"`), the synthetic-message workaround for Notes | `sync_mail()`, `sync_contacts_collection()`, `sync_calendar_collection()`, `sync_notes_collection()`, `sync_tasks_collection()` (all in `activesync.rs`). Contacts and Calendar were read-only as of an earlier pass (verified by grep: no `SyncClientCommandKind` match in either function) — both rewritten overnight to the same Add/Change/Delete + hash-diff pattern Notes already used, backed by `ContactCard/set`/`CalendarEvent/set`. Both confirmed live to support real in-place `update` (unlike `Email/set`), so neither needed Notes' stable-id-via-keyword workaround. Full Add→Change→Delete lifecycle live-verified for both with throwaway test items. Mail's own client-Add (e.g. append-to-Sent) is still explicitly ignored (`apply_mail_client_commands`, logged debug, no-op) — real EAS clients rarely Add mail directly since SendMail covers the compose path, low priority. Server-side mail deletions are now also surfaced as ActiveSync deletes: `emails_still_in_mailbox()` (`jmap/client.rs`) confirms each previously-seen-but-absent-from-this-window id directly via `Email/get`, rather than assuming absence-from-window means deleted — `emails_in_mailbox`'s query is sorted newest-first and capped at the window size, so an old message pushed out of the window by newer mail looks identical to a genuinely deleted one under a naive diff; this was a real correctness risk in the naive approach, not a hypothetical one. |
